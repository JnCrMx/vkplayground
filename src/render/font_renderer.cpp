...
namespace render
{
	...
	void font_renderer::preload(FT_Library ft, resource_loader* loader, vk::RenderPass renderPass)
	{
		...
		{
			vk::UniqueShaderModule vertexShader = createShader(device, "text.vert");
			vk::UniqueShaderModule geometryShader = createShader(device, "text.geom");
			vk::UniqueShaderModule fragmentShader = createShader(device, "text.frag");
